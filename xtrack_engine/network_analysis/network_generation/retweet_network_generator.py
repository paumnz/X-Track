"""
Module to implement the retweet network extraction functionality of XTRACK framework's engine.
"""
...
class RetweetNetworkGenerator(NetworkGenerator):
    """
    A class to implement the retweet network extraction functionality of XTrack's engine.
    """
    ...
    def _generate_network_dataframe(
        ...
        """
        Private method to generate a Pandas DataFrame with retweet relationships from the given campaign and hashtags (if provided).
        ...
        Returns:
            A pandas DataFrame with the retweet relationships between users and their strengths.
        """
        self.logger.debug(f'Retrieving retweet network from {first_date} to {last_date}')
        ...
        if hashtags is None:
            query = """
                ...
                FROM
                    ...
                    INNER JOIN tweet t_rted ON t_rted.tweet_id = rt.retweeted
